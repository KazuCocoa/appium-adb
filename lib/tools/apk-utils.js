...
apkUtilsMethods.installOrUpgrade = async function (apk, pkg = null, options = {}) {
  if (path.extname(apk) === '.aab') {
    // TODO: Take care of Windows, options
    const apksPath = '/tmp/appium-test.apks';
    await bundletool.buildApks(apk, this.curDeviceId, apksPath, {overwrite: true});
    return await bundletool.installApks(apksPath, this.curDeviceId);
  }
